...
describe Unloq::Client, '#lookup_event', vcr: true do
  let(:instance) { Unloq::Client.new(api_key: TestCredentials.api_key, namespace: TestCredentials.namespace) }
  subject { instance.lookup_event(author, verb, recipient) }
  ...
  context "the event does not exist" do
    let(:verb) { "not-at-all-a-real-thing" }
